...
def ger_usuarios():
    ...
    if search_query:
        ...
        query = query.filter(
            db.or_(Usuario.nome.ilike(search_term), Usuario.email.ilike(search_term))
        )
...
def criar_usuario():
    ...
    if not form.validate_on_submit():
        ...
        return (
            jsonify({"mensagem": "Dados inválidos", "erros": erros}),
            400,  # 400 Bad Request
        )
...
def editar_usuario(usuario_id):
    ...
    # Remove a validação da senha se o campo não for preenchido
